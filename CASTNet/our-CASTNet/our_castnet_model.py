...
class CASTNet:
    ...
    def __init__(self, sess, conf):
        ...
        with tf.compat.v1.variable_scope('placeholders'):
            self.svi_input_local = tf.compat.v1.placeholder(tf.float32, [None, self.TIME_STEPS, self.TEMPORAL_FEATURE_SIZE], name='svi_local_x')
            self.svi_input_global = tf.compat.v1.placeholder(tf.float32, [None, self.TIME_STEPS, self.NO_LOCATIONS, self.TEMPORAL_FEATURE_SIZE], name='svi_global_x')
            ...
            self.target = tf.compat.v1.placeholder(tf.float32, [None], name='target_y')
        ...
        with tf.compat.v1.variable_scope('static_variables'):
        ...
        with tf.compat.v1.variable_scope('local_variables'):
            self.local_lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self.HIDDEN_LAYER_SIZE)
            self.local_lstm_cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(self.local_lstm_cell, output_keep_prob=(1.0 - self.keep_prob))
        
        with tf.compat.v1.variable_scope('global_variables'):
            ...
            for i in range(0, self.NO_SPATIAL_HEADS):
                ...
                global_spatial_w_weight = tf.compat.v1.get_variable(name='global_spatial_att_w_weight_' + str(i), 
            ...
            for i in range(0, self.NO_SPATIAL_HEADS):
                ...
                global_lstm_cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(global_lstm_cell, output_keep_prob=(1.0 - self.keep_prob)) 
                self.global_lstm_cells.append(global_lstm_cell)
        
        
        with tf.compat.v1.variable_scope('spatiotemporal_variables'):
            ...
            self.spatiotemporal_att_w_weight = tf.compat.v1.get_variable(name='spatiotemporal_att_w_weight', 
                                                               shape=[self.HIDDEN_LAYER_SIZE], 
                                                               regularizer = None)
        
        with tf.compat.v1.variable_scope('local_global_att_variables'):
            ...
            self.local_global_att_w_weight = tf.compat.v1.get_variable(name='local_global_att_w_weight', 
                                                               shape=[self.HIDDEN_LAYER_SIZE], 
                                                               regularizer = None)
        
        
        with tf.compat.v1.variable_scope('temporal_att_variables'):
            ...
            for i in range(0, self.NO_SPATIAL_HEADS):
                ...
                global_temporal_att_w_weight = tf.compat.v1.get_variable(name='global_temporal_att_w_weight_' + str(i), 
                                                             shape=[self.HIDDEN_LAYER_SIZE], 
                                                             regularizer = None)
            ...
            self.local_temporal_att_w_weight = tf.compat.v1.get_variable(name='local_temporal_att_w_weight', 
                                                             shape=[self.HIDDEN_LAYER_SIZE], 
                                                             regularizer = None)
    ...
    def _getGlobalOutputs(self):
        ...
        for i in range(0, self.NO_SPATIAL_HEADS):
            M = tf.tanh(tf.einsum("baij,jk->baik", self.svi_input_global, self.global_spatial_att_input_weights[i]))   #(B, W, L, D)
            ...
            r = tf.einsum("baij,bai->baj", self.svi_input_global, a)   #(B, W, D)
        ...
        for i in range(0, self.NO_SPATIAL_HEADS):
            with tf.compat.v1.variable_scope(str(i) + '_global_lstm_variables'):
                x_global = tf.unstack(self.unstacked_global_spatial_att_outputs_arr[i], self.TIME_STEPS, 1)
                global_outputs, global_states = tf.compat.v1.nn.static_rnn(self.global_lstm_cells[i], x_global, dtype=tf.float32)
    ...
    def _getLocalOutputs(self):
        ...
        with tf.compat.v1.variable_scope('local_lstm_variables'):
            x_local = tf.unstack(self.svi_input_local, self.TIME_STEPS, 1)
    ...
    def _loss(self, outputs):
        ...
        self.regression_loss = tf.compat.v1.losses.mean_squared_error(self.target, tf.squeeze(outputs))
            ...
            for i in indices:
                ...
                if ((global_step+1) % self.DISP_ITER) == 0:
                    ...
                    if(self.TEST_TIME == False):
                        valid_preds = self.predict(valid_svi_local, valid_svi_global, valid_static, valid_sample_indices, valid_dist, valid_y)
                        self.results['preds'].append(valid_preds)
                    elif(self.TEST_TIME == True):
                        test_preds = self.predict(test_svi_local, test_svi_global, test_static, test_sample_indices, test_dist, test_y)
    ...
    def predict(self, svi_data_local, svi_data_global, static_data, sample_indices, dist_data, target_data):
        ...
        feed_dict = {
                    self.svi_input_local: svi_data_local,
                    self.svi_input_global: svi_data_global,
